...
function buildmodel(data_file)
    ...
    @objective(m, Min,
        ...
        + (tAC * sum(TransCap[r1, r2] for r1 in REGION for r2 in REGION))/2
    )
    ...
    #3
    @constraint(m, Prod[r in REGION, h in HOUR],
        sum(Electricity[r, :, h]) + 0.9*Out[r, h] - In[r, h] + sum(0.98*Transmission[r2, r, h]-Transmission[r, r2, h] for r2 in REGION) >= load[r, h]
    )
    ...
    #3
    @constraint(m, ZeroTransCaps[r in REGION], TransCap[r, r] == 0)
    @constraint(m, EqualTransCaps[r1 in REGION, r2 in REGION], TransCap[r1, r2] == TransCap[r2, r1])
    @constraint(m, TransHourCaps[r1 in REGION, r2 in REGION, h in HOUR], Transmission[r1, r2, h] + Transmission[r2, r1, h] <= TransCap[r1, r2])

    return m, Capacity, Electricity, Batteries, Transmission, TransCap
end