...
function buildmodel(data_file)
    ...
    #3
    #@variable(m, Transmission[r1 in REGION, r2 in REGION] >= 0)
   ...
   # @variable(m, Out[r in REGION, h in HOUR]>=0)
    ...
    @objective(m, Min,
        ...
        #+ bAC*sum(Batteries[r] for r in REGION) + sum(BatteriesCharge[r,h]*0.1 for r in REGION for h in HOUR) + (tAC * sum(Transmission[r1,r2] for r1 in REGION for r2 in REGION))/2
    )
